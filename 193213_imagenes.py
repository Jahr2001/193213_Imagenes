...
def descarga_url_img(link):
   ...
   url_local = "D:\\Documentos\\7mo Cuatrimestre\\Programación Concurrente\\Segundo Corte\\193213_Imagenes\\Imagenes\\{}.{}"
...
def subprocesoMultiples():
    print('\n Subprocesos')
    with ThreadPoolExecutor(max_workers=len(listaUrls)) as executer: # max_work es un numero de hilos
...
def multiprocesamiento():
    print('\n Multiprocesamiento')
    pool = Pool(len(listaUrls))
    pool.map(descarga_url_img, listaUrls)
